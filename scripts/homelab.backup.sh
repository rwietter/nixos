...
for db_file in "${DB_FILES[@]}"; do
  if [ -f "$db_file" ]; then
    ...
    echo "Backing up ${original_filename} from ${app_name} into folder..."
...
echo "${#app_directories[@]} database files found for backup."

if [ ${#app_directories[@]} -eq 0 ]; then
  # Disable the trap so it doesn't notify failure in an expected case
  echo "No database files found for backup. Exiting."
  trap - EXIT
  exit 0
fi
